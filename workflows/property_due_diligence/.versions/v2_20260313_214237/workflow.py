...
try:
    ...
    # Fallback for development/testing without BeeAI
    BEEAI_AVAILABLE = False
    print("Warning: BeeAI Framework not available. Workflow will run in simulation mode.")
...
class PropertyDueDiligenceWorkflow:
    ...
    def __init__(self):
        ...
        if BEEAI_AVAILABLE:
            self.workflow = Workflow(PropertyDueDiligenceState, name="PropertyDueDiligenceWorkflow")
    ...
    def _register_steps(self):
        ...
        self.workflow.add_step("get_property_details", self.get_property_details)
        self.workflow.add_step("parallel_research", self.parallel_research)
        self.workflow.add_step("assess_risks", self.assess_risks)
        self.workflow.add_step("generate_report", self.generate_report)
    ...
    async def generate_report(self, state: PropertyDueDiligenceState) -> str:
        """
    ...
    async def run(self, property_id: int):
        """
        Execute the complete workflow.
        
        Args:
            property_id: Property ID to research
        
        Returns:
            Workflow result with final state
        
        Example:
            workflow = PropertyDueDiligenceWorkflow()
            result = await workflow.run(property_id=12345)
            
            print(f"Risk Level: {result.state.risk_level}")
            print(f"Summary: {result.state.findings_summary}")
        """
        # Build initial state
        initial_state = PropertyDueDiligenceState(property_id=property_id)
        
        print("=" * 80)
        print("PROPERTY DUE DILIGENCE WORKFLOW")
        print("=" * 80)
        ...
        if BEEAI_AVAILABLE:
            ...
            # Simulation mode: Execute steps manually
            print("Running in simulation mode (BeeAI Framework not available)")
            state = initial_state
            current_step = "get_property_details"
            ...
            while current_step and current_step != "END":
                step_func = step_map.get(current_step)
                if step_func:
                    current_step = await step_func(state)
                else:
                    break
...
async def main():
    ...
    print("FINAL RESULTS")
    print("=" * 80)
    print(f"\nRisk Level: {result.state.risk_level.upper()}")
    print(f"Checks Performed: {', '.join(result.state.checks_performed)}")
    
    if result.state.risk_factors:
        print(f"\nRisk Factors:")
        for factor in result.state.risk_factors:
            print(f"  - {factor}")
    
    print(f"\n{'-'*80}")
    print("FULL REPORT:")
    print(f"{'-'*80}")
    print(result.state.findings_summary)
