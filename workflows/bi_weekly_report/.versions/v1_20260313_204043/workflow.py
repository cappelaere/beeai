...
class BiWeeklyReportWorkflow:
    """Flo workflow: BI Weekly Report (9 steps)."""

    def __init__(self, run_id: str | None = None):
        self.run_id = run_id
        self.workflow = Workflow(BiWeeklyReportState, name="BiWeeklyReportWorkflow")
        self._register_steps()

    def _register_steps(self):
        self.workflow.add_step("get_active_properties", self.get_active_properties)
        self.workflow.add_step("collect_portfolio_metrics", self.collect_portfolio_metrics)
        self.workflow.add_step("identify_top_performers", self.identify_top_performers)
        self.workflow.add_step("identify_properties_needing_attention", self.identify_properties_needing_attention)
        self.workflow.add_step("conversion_funnel_analysis", self.conversion_funnel_analysis)
        self.workflow.add_step("week_over_week_trend", self.week_over_week_trend)
        self.workflow.add_step("auction_readiness_snapshot", self.auction_readiness_snapshot)
        self.workflow.add_step("generate_recommended_actions", self.generate_recommended_actions)
        self.workflow.add_step("produce_executive_brief", self.produce_executive_brief)

    async def get_active_properties(self, state: BiWeeklyReportState) -> str:
    ...
    async def run(self, start_date: str, end_date: str):
        initial_state = BiWeeklyReportState(start_date=start_date, end_date=end_date)
