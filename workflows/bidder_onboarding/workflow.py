...
class BidderOnboardingWorkflow:
    ...
    async def check_sam_compliance(self, state: BidderOnboardingState) -> str:
        ...
        # Match bidder name against name, first, middle, last, and cross_reference (aliases).
        # CSV often has individuals in Last only; names may be stored with surrounding quotes.
        search_name = (state.bidder_name or "").strip()
        if not search_name:
            search_term = "%"
        else:
            search_term = f"%{search_name}%"
        params = (search_term,) * 5  # name, first, middle, last, cross_reference
        ...
        # Branch: if SAM failed, skip OFAC and go to finalize_status
        if not state.sam_passed:
            return "finalize_status"
