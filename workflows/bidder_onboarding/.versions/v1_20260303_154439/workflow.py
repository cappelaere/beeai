...
class BidderOnboardingWorkflow:
    ...
    def __init__(self, strict_mode: bool = False, run_id: str = None):
        ...
        self.workflow = Workflow(BidderOnboardingState, name="BidderOnboardingWorkflow")
        self._register_steps()
    
    def _register_steps(self):
        """Register all workflow steps in execution order."""
        self.workflow.add_step("validate_input", self.validate_input)
        self.workflow.add_step("check_property_status", self.check_property_status)
        self.workflow.add_step("check_sam_compliance", self.check_sam_compliance)
        self.workflow.add_step("check_ofac_compliance", self.check_ofac_compliance)
        self.workflow.add_step("determine_eligibility", self.determine_eligibility)
        self.workflow.add_step("request_human_review", self.request_human_review)
        self.workflow.add_step("process_human_review", self.process_human_review)
        self.workflow.add_step("finalize_status", self.finalize_status)
        self.workflow.add_step("create_audit_log", self.create_audit_log)
    ...
    async def run(
        ...
        age_accepted: bool | None = None,
        registration_data: dict | None = None
    ):
        """
        Execute the Bidder Onboarding Workflow.
        
        Args:
            bidder_name: Full name of bidder to screen
            property_id: Property ID for registration
            email: Bidder's email address
            phone: Contact phone number
            user_type: Type of bidder (Investor, Agent, Individual, Corporation)
            terms_accepted: Terms and conditions accepted
            age_accepted: Age verification accepted
            registration_data: Additional registration form data (optional, merged with above fields)
        ...
        """
        # Build registration_data from individual parameters if provided
        if registration_data is None:
            registration_data = {}
        
        # Merge individual parameters into registration_data (individual params take precedence)
        if email is not None:
            registration_data['email'] = email
        if phone is not None:
            registration_data['phone'] = phone
        if user_type is not None:
            registration_data['user_type'] = user_type
        if terms_accepted is not None:
            registration_data['terms_accepted'] = terms_accepted
        if age_accepted is not None:
            registration_data['age_accepted'] = age_accepted
        
        # Set defaults if not provided
        registration_data.setdefault('email', 'test@example.com')
        registration_data.setdefault('terms_accepted', True)
        registration_data.setdefault('age_accepted', True)
        
        initial_state = BidderOnboardingState(
        ...
        print("=" * 80)
        print("BIDDER ONBOARDING & VERIFICATION WORKFLOW")
        print("=" * 80)
...
async def main():
    """
    Standalone execution example for testing the workflow.
    """
    workflow = BidderOnboardingWorkflow(strict_mode=False)
    
    # Test case 1: Clean bidder (should be approved)
    ...
    print("=" * 80)
    print(f"Final Status: {result1.state.approval_status}")
    print(f"Eligible: {result1.state.is_eligible}")
    print(f"Requires Review: {result1.state.requires_manual_review}")
    print(f"Risk Factors: {result1.state.risk_factors}")
    print(f"Compliance Summary: {result1.state.compliance_summary}")
    if result1.state.bid_limit:
        print(f"Bid Limit: ${result1.state.bid_limit:,.2f}")
    print(f"Steps Executed: {len(result1.state.workflow_steps)}")
    print(f"Checks Performed: {len(result1.state.checks_performed)}")
    print("=" * 80)
