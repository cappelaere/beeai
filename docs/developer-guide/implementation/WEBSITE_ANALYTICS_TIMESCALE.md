...
- Visitor/session/user attribution:
  - `visitor_id` (random per-session pseudonymous identifier)
  - `session_key_hash` (SHA-256 hash only, no raw session key)
...
The endpoint is CSRF-protected.
...
- SQLite/non-PostgreSQL environments no-op safely.
- PostgreSQL environments without the Timescale extension available are skipped (non-blocking).
- You can disable setup explicitly with `WEBSITE_ANALYTICS_TIMESCALE_ENABLED=0`.
