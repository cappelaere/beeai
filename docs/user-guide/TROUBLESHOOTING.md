...
### Session export / PDF export

#### Issue: "Download as PDF" returns 500 or error; log shows "cannot load library 'libgobject-2.0-0'"

**Symptoms:**
- Export session → "Download as PDF" fails or returns a server error.
- Server log: `PDF export failed: cannot load library 'libgobject-2.0-0'` (or similar `libpango`, `libgobject` messages).

**Cause:** PDF export uses [WeasyPrint](https://weasyprint.org/), which on macOS needs system libraries (Pango, GObject) that are not installed by `pip install weasyprint` alone.

**Solution (macOS):**
```bash
# Install Pango and related libraries used by WeasyPrint
brew install pango
```

If you still see missing library errors, install the full set:
```bash
brew install pango gdk-pixbuf libffi
```

Then restart the app and try "Download as PDF" again. Markdown and "View in browser" export do not require these libraries.

**References:** [WeasyPrint — First steps](https://doc.courtbouillon.org/weasyprint/stable/first_steps.html#installation), [Troubleshooting](https://doc.courtbouillon.org/weasyprint/stable/first_steps.html#troubleshooting).
